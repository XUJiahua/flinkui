...
（records in/out、backpressure、checkpoint duration/size、lag）。

**代码依据**：
- `internal/api/proxy.go` — 已能反代 JM REST,可顺手拉 `/jobs/{id}/metrics`、`/checkpoints`
- `internal/api/handlers.go` — `readyz` 恒 ready,连不上 K8s 时无 degraded 信号

**建议做法**：复用已有 JM 反代,从 `/jobs/{id}/metrics`、`/checkpoints` 拉关键指标做成
dashboard 卡片,无需引入完整 Prometheus;并给 `readyz` 增加 degraded 语义。

**验收**：作业卡片展示 checkpoint / 吞吐 / backpressure 关键指标。
