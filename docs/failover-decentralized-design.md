...
- **Fencing token**（沿用现状，key **每组独立**，默认 `fencing/<group>/active-cluster`）：
  内容为当前活跃
  `clusterId`，或中性值 `__switching__`（两侧都被 fence）。
- **交接记录**（新，JSON，key 默认 `fencing/<group>/handoff`）：
  ```json
  {
    "group": "orders",
...
`handoff_key` 就是这条留言在共享 S3 上的对象键,默认 `fencing/<组名>/handoff`,一般不用改。

### 3.2 fencing key 必须**每组独立**,且要与作业 initContainer 对齐(重要)

平台按**每组(每个作业)独立**做主从,因此 `fencing_key` 默认是 **`fencing/<组名>/active-cluster`**,
一组一个 key。**不要让多组共用同一个 fencing key**——否则 Release 一个作业会把共享 token 置为
中性,导致**所有组都显示 neutral**、并且 Promote 其它(未让位的)组会被"未让位"拦住。

关键对齐:平台读写的 `fencing_key` **必须等于该作业 Pod 的 fencing initContainer 实际校验的 key**。
两种模型二选一,保持两端一致:
- **每作业 fencing(推荐,平台默认)**:每个作业的 initContainer 校验自己的 key(如
  `fencing/<job>/active-cluster`);平台每组用同名 key。各作业独立切换,互不影响。
- **整集群 fencing(旧 failover.sh 单作业环境的隐含用法)**:所有作业共用一个 key
  (如 `fencing/active-cluster`)。此时切换是**整集群**语义——Release/Promote 会影响所有作业的
  token。若确实要这样,把每组 `fencing_key` 显式设成同一个共享值,并理解"动一个=动全部"。
...
ha:
  groups:
    - name: orders
      ...
      fencing_key: fencing/orders/active-cluster   # 可选，默认 fencing/<组名>/active-cluster（每组独立）
      neutral_token: __switching__          # 可选，默认
      handoff_key: fencing/orders/handoff   # 可选，默认 fencing/<组名>/handoff
```
